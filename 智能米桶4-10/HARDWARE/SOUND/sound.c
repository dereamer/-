...
{	 
	...
	GPIO_InitTypeDef  GPIO_InitStructure;
 	...
 	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_4|GPIO_Pin_8|GPIO_Pin_13;//PF6,PF7
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
	GPIO_Init(GPIOA, &GPIO_InitStructure);	
	
	GPIO_SetBits(GPIOA,GPIO_Pin_4|GPIO_Pin_8|GPIO_Pin_13|GPIO_Pin_12);
...
void mp3_play_begin(void)
{
  u8 res;
	...
	u8 *fn;   			//长文件名
	u8 *pname;
 	f_opendir(&mp3dir,"0:/MUSIC");//打开图片文件夹									  										   
  mp3fileinfo.lfsize=_MAX_LFN*2+1;				//长文件名最大长度
	...
	VS_HD_Reset();
	VS_Soft_Reset();
	vsset.mvol=254;						//默认设置音量为210.
  res=f_opendir(&mp3dir,(const TCHAR*)"0:/MUSIC"); 	//打开目录
	mp3_play_song("0:/MUSIC/test1.mp3");  											  
...
void mp3_play_end(void)
{
  ...
  mp3fileinfo.lfsize=_MAX_LFN*2+1;				//长文件名最大长度
	...
	vsset.mvol=254;						//默认设置音量为210.
  res=f_opendir(&mp3dir,(const TCHAR*)"0:/MUSIC"); 	//打开目录
	mp3_play_song("0:/MUSIC/test2.mp3");  											  
	myfree(mp3fileinfo.lfname);	//释放内存			    
	myfree(pname);				//释放内存			    
